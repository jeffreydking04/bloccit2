...
RSpec.describe FavoritesController, type: :controller do
  ...
  context 'signed in user' do
    ...
    describe 'POST create' do
      it 'should redirect to the posts show view' do
        post :create, { post_id: my_post.id }
        expect(response).to redirect_to([my_topic, my_post])
      end

      it 'should create a favorite for the current user and specified post' do
    ...
    describe 'DELETE destroy' do
      ...
      it 'should destroy the favorite for the current user and post' do
        favorite = my_user.favorites.where(post: my_post).create
        expect( my_user.favorites.find_by_post_id(my_post.id) ).not_to be_nil
